...
class App:
    ...
    def fn_menu(self, list_menu):
        ...
        match status:
            case 1:
                init_app = fnht.InitDB()
                init_app.fn_init_db()
                return True
            case 2:
                create_app = fnht.CreateDB()
                boucle = True
                while boucle:
                    boucle = create_app.fn_create_db()
                return True
            case 3:
                read_app = fnht.ReadDB()
                read_app.fn_read_db()
                return True
            case 4:
                update_app = fnht.UpdateDB()
                update_app.fn_update_db()
                return True
            case 5:
                delete_app = fnht.DeleteDB()
    ...
    def fn_start(self):
        boucle = True
        list_menu = self.fn_init_menu()
        while boucle:
            boucle = self.fn_menu(list_menu)
